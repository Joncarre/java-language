...
public class Main {
	...
	private static <S extends GameState<S, A>, A extends GameAction<S, A>> void startGUIMode(
		...
		for (int i = 0; i < game.getState().getPlayerCount(); i++){
			...
			final ConcurrentAiPlayer player2;
			player1 = new RandomPlayer("Player " + i);
			player2 = new ConcurrentAiPlayer("Concurrent Player");
			player1.join(i);
			player2.join(i);
			...
			try{
				SwingUtilities.invokeAndWait(new Runnable() {
					...
					@Override
					public void run() { 
						GameController<S, A> gameCtrl = new GUIController<S, A>(j, player1, player2, game, executor);
		...
		executor.submit(new Runnable() {
			...
			public void run() {
				game.start();
				usage(); // Muestra una pequeña ayuda
			}
		});
	}
	...
	private static void usage(){
		System.out.println("Ayuda para el parseo: \n \n"
				+ "[game] [mode] [player1] [player2] \n \n"
				+ "game: ttt o was \n"
				+ "mode: gui o console \n"
				+ "player: manual, random o smart");
	}
}
